...
@app.route('/admin/cleanup-models', methods=['POST'])
def cleanup_orphaned_models():
    """Trigger cleanup of models that are no longer in MinIO storage"""
    try:
        # Check if user confirmed the action
        confirmation = request.json.get('confirm') if request.is_json else request.form.get('confirm')
        
        if not confirmation or confirmation.lower() != 'yes':
            return jsonify({
                'error': 'Confirmation required. Send {"confirm": "yes"} to proceed.',
                'warning': 'This action will remove models from the server that are no longer available in MinIO storage. This is irreversible unless the model is re-uploaded to the registry.'
            }), 400
        
        # Create signal file for the sidecar
        models_path = '/models'  # This should match the sidecar's models path
        cleanup_signal_file = os.path.join(models_path, '.cleanup_models')
        ...
        try:
            ...
            with open(cleanup_signal_file, 'w') as f:
                f.write(f"cleanup_requested_at_{datetime.now().isoformat()}")
            
            log_processing_step("Model Cleanup", "Cleanup signal sent to sidecar", "info")
            
            return jsonify({
                'success': True,
                'message': 'Model cleanup signal sent to sidecar. Orphaned models will be removed in the next sync cycle.',
                'warning': 'Models removed from MinIO storage will be deleted from the server.'
            })
            
        except Exception as e:
            log_processing_step("Model Cleanup", f"Failed to create cleanup signal: {str(e)}", "error")
            return jsonify({
                'error': f'Failed to signal cleanup: {str(e)}'
            }), 500
            
    except Exception as e:
        log_processing_step("Model Cleanup", f"Error in cleanup endpoint: {str(e)}", "error")
        return jsonify({'error': str(e)}), 500

@app.route('/admin/models-status')
def get_models_status():
    """Get detailed status of models including MinIO vs local sync status"""
    try:
        # Get currently available models from OVMS
        available_models = client.get_available_models()
        
        # Try to get MinIO models list (this would need to be implemented in the sidecar)
        models_status = {
            'ovms_models': available_models,
            'total_ovms_models': len(available_models),
            'timestamp': datetime.now().isoformat(),
            'warning': 'To see MinIO sync status, ensure the sidecar is running and accessible.'
        }
        
        # Check if models list file exists (created by sidecar)
        models_list_file = '/models/models.json'
        if os.path.exists(models_list_file):
            try:
                with open(models_list_file, 'r') as f:
                    sidecar_info = json.load(f)
                    models_status['sidecar_models'] = sidecar_info.get('models', [])
                    models_status['sidecar_updated_at'] = sidecar_info.get('updated_at', 'unknown')
                    models_status['total_sidecar_models'] = len(sidecar_info.get('models', []))
                    
                    # Compare lists
                    ovms_set = set(available_models)
                    sidecar_set = set(sidecar_info.get('models', []))
                    
                    models_status['sync_status'] = {
                        'in_sync': ovms_set == sidecar_set,
                        'only_in_ovms': list(ovms_set - sidecar_set),
                        'only_in_sidecar': list(sidecar_set - ovms_set)
                    }
                    
            except Exception as e:
                models_status['sidecar_error'] = f"Failed to read sidecar models list: {str(e)}"
        
        log_processing_step("Models Status", f"Retrieved status for {len(available_models)} models", "success")
        
        return jsonify(models_status)
        
    except Exception as e:
        log_processing_step("Models Status", f"Error getting models status: {str(e)}", "error")
        return jsonify({'error': str(e)}), 500
