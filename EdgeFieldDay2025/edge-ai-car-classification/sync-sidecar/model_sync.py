...
class ModelSyncSidecar:
    ...
    def cleanup_orphaned_models(self) -> bool:
        """Remove models from local storage and OVMS config that are no longer in MinIO."""
        try:
            # Get models currently in MinIO
            minio_models = self.discover_models_in_minio()
            minio_model_names = {model_info['name'] for model_info in minio_models}
            
            # Get models currently configured in OVMS
            configured_models = set(self.ovms_config.get_configured_models())
            
            # Find orphaned models (configured but not in MinIO)
            orphaned_models = configured_models - minio_model_names
            
            if not orphaned_models:
                self.logger.info("No orphaned models found")
                return True
            
            self.logger.info(f"Found {len(orphaned_models)} orphaned models: {orphaned_models}")
            
            cleanup_success = True
            for model_name in orphaned_models:
                try:
                    # Remove from OVMS configuration
                    if self.ovms_config.remove_model(model_name):
                        self.logger.info(f"Removed {model_name} from OVMS config")
                    
                    # Remove local model files
                    model_path = os.path.join(self.models_path, model_name)
                    if os.path.exists(model_path):
                        shutil.rmtree(model_path)
                        self.logger.info(f"Removed local model files for {model_name}")
                    
                except Exception as e:
                    self.logger.error(f"Failed to cleanup model {model_name}: {str(e)}")
                    cleanup_success = False
            
            # Save updated OVMS configuration
            if self.ovms_config.save_config():
                self.logger.info("Updated OVMS configuration after cleanup")
            else:
                self.logger.error("Failed to save OVMS configuration after cleanup")
                cleanup_success = False
            
            return cleanup_success
            
        except Exception as e:
            self.logger.error(f"Failed to cleanup orphaned models: {str(e)}")
            return False
    
    def force_cleanup_all_models(self) -> bool:
        """Remove all models from local storage and OVMS config (for testing/reset)."""
        try:
            configured_models = self.ovms_config.get_configured_models()
            
            if not configured_models:
                self.logger.info("No models to cleanup")
                return True
            
            self.logger.warning(f"Force cleanup: removing all {len(configured_models)} models")
            ...
            # Save updated OVMS configuration
            if self.ovms_config.save_config():
                self.logger.info("Updated OVMS configuration after force cleanup")
            else:
                self.logger.error("Failed to save OVMS configuration after force cleanup")
                cleanup_success = False
            
            return cleanup_success
            
        except Exception as e:
            self.logger.error(f"Failed to force cleanup all models: {str(e)}")
            return False
    ...
    def run_continuous_sync(self):
        ...
        # Signal files for external commands
        cleanup_signal_file = os.path.join(self.models_path, ".cleanup_models")
        force_cleanup_signal_file = os.path.join(self.models_path, ".force_cleanup_models")
        ...
        while True:
            try:
                ...
                if os.path.exists(cleanup_signal_file):
                    self.logger.info("Cleanup signal detected, removing orphaned models...")
                    try:
                        self.cleanup_orphaned_models()
                        os.remove(cleanup_signal_file)
                        self.logger.info("Cleanup signal processed and removed")
                    except Exception as e:
                        self.logger.error(f"Error processing cleanup signal: {str(e)}")
                        # Remove signal file even if cleanup failed to prevent infinite loop
                        try:
                            os.remove(cleanup_signal_file)
                        except:
                            pass
                
                # Check for force cleanup signal
                if os.path.exists(force_cleanup_signal_file):
                    self.logger.warning("Force cleanup signal detected, removing all models...")
                    try:
                        self.force_cleanup_all_models()
                        os.remove(force_cleanup_signal_file)
                        self.logger.warning("Force cleanup signal processed and removed")
                    except Exception as e:
                        self.logger.error(f"Error processing force cleanup signal: {str(e)}")
                        # Remove signal file even if cleanup failed to prevent infinite loop
                        try:
                            os.remove(force_cleanup_signal_file)
                        except:
