...
add_to_fstab() {
    ...
    then
        ...
        LINE="${UUID}\t${MOUNTPOINT}\text4\tdefaults\t0 0"
...
do
    ...
    add_to_fstab "${PARTITION}" "${MOUNTPOINT}"
    echo "Mounting disk ${PARTITION} on ${MOUNTPOINT}"
    mount "${MOUNTPOINT}"
done

###reaplce UUID with disk name ###
newline="/dev/vda1       /               ext4    errors=remount-ro 0       1"
sed -i "s#^UUID=.*#$newline#" /etc/fstab
